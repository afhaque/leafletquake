...
$.getJSON("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_week.geojson",function(data){
	...
	// Function that returns an interval of time during which the earthquake will be visible on the map.
	var getInterval = function(quake) {
		// We need a start and end time. 
		// Since we only have a start time we create an end time by adding a time start based on the magnitude of the earthquake.
      	return {
    ...
    // Creates a timeline control object on our map.
    var timelineControl = L.timelineSliderControl({
      	formatOutput: function(date) {
       		return new Date(date).toString();
      	},
      	// The number of 'ticks' or frames that will be displayed on our map.
      	steps: 2000
    });

    // Creating the timeline layer.
    var timeline = L.timeline(data, {
      	...
      	pointToLayer: function(data, latlng){
        	return L.circleMarker(latlng, {
          		...
          		fillOpacity: .75,
          		weight: 0
        	}).bindPopup('Magnitude: ' + data.properties.mag + '<br>' + 'Location: ' + data.properties.place);
      	}
    });
